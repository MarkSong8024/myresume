...
FileProgress.prototype.setComplete = function (up, info) {
    ...
    if (res.url) {
        url = res.url;
        str = "<div style='display: none'><strong>Link:</strong><a href=" + res.url + " target='_blank' > " + res.url + "</a></div>" +
    ...
    //预览效果
    //var imageView = '?imageView2/1/w/200/h/200';
    var imageView = '?imageMogr2/auto-orient/thumbnail/200x';
    ...
    function addImgInfo(url) {

        var strUrl = "/exam/question!savePicName.action?appKey=" + key_qinniu + "&queId=" + id_qiniu + "&str=";
        var str = url.split('?')[0];
        $.ajax({
            type: "get",
            url: strUrl + str,
    ...
    if (!isImg) {
        ...
        $(img).on('load', function () {
            ...
            //隐藏查看处理效果
            var infoWrapper = $('<div style="display: none " class="infoWrapper col-md-6"></div>');
